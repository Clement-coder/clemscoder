  ...
  return (
    ...
    >
      <nav className="flex items-center justify-between max-w-[1600px] mx-auto">
        ...
        {/* Mobile menu — AnimatePresence enables exit animation */}
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              initial={{ x: '100%' }}
              ...
              className="fixed inset-0 bg-light-navy flex flex-col items-center justify-center z-40"
            >
              <ol className="flex flex-col items-center gap-8 list-none font-mono text-sm mb-8">
                {navLinks.map(({ name, url }, i) => (
                  <li key={name}>
                    <a
                      href={url}
                    ...
                    >
                      <div className="text-green text-sm mb-1 text-center">0{i + 1}.</div>
                      {name}
                    </a>
                  </li>
                ))}
